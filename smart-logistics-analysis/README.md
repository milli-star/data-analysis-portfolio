...
일반적으로 이진 타겟 변수(Logistics_Delay)가 존재하면 분류 모델링으로 이어지는 경우가 많으나, 본 분석에서는 모델링을 진행하지 않았다. 그 이유는 다음과 같다.

Traffic_Status를 제외한 7개 변수(Temperature, Humidity, Asset_Utilization, Inventory_Level, Demand_Forecast, 시간대, 요일)를 상관계수·구간별 비교·히트맵으로 교차 검증한 결과, 모든 변수가 지연 여부와 통계적으로 무관함을 확인했다(상관계수 전부 ±0.08 이내). 반면 Traffic_Status는 Heavy(정체) 상태일 때 지연 비율이 100%로, 단일 조건만으로 완전한 설명력을 가졌다.

이런 조건에서 머신러닝 모델을 적용하면 다수 변수의 노이즈만 추가될 뿐, "Traffic_Status가 Heavy인가"라는 단일 규칙 이상의 예측력을 얻기 어렵다. 따라서 모델링보다 변수 간 관계를 면밀히 검증하는 것이 이번 분석에서는 더 우선순위가 높다고 판단했으며, 이는 데이터의 특성을 충분히 파악한 후 분석 방법을 선택하는 과정으로 이해할 수 있다.